...
public interface IPostagemRepository
{
    ...
    Task<PagedList<List<PostagemEntity>?>> GetAllAsync (CancellationToken token);
    Task<Result<PostagemEntity>> UpdateAsync (PostagemEntity entity, CancellationToken token);
    Task<PagedList<List<PostagemEntity>?>> SearchAsync (Expression<Func<PostagemEntity, bool>> expression, string entity, CancellationToken token);
}
